...
3. Time complexities for methods
   ...
   isEmpty: O(1). Evaluates whether the size is 0 or not
...
4. When push is called when the stack is full, the program prints an error message to the console and returns without
   changing topIndex. Typically, asserts or a try-catch might be used, but in my desperation to avoid STL or try-catches,
   the program continues without crashing by returning from the push method, having accomplished nothing if the stack is
...
7. An advantage of a linked list is that one could actually change what memory address a pointer in a linked list is
   pointing to, rather than just an index. The disadvantage of pointers and linked lists is that we would need to manage
